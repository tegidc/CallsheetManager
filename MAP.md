...
- `renderProjectOverview()` — renders the project overview tab body: Project details, then Quick add, then Tasks, then AI Scan, then Danger zone (Phase Overview Reorder — Tasks/AI Scan used to render above Project details; Phase Quick Add inserted Quick add between details and Tasks, as manual data entry rather than an assistance feature). Project details has no manual Save button — autosave (`body.oninput` → `scheduleAutosave('overview', …)` → `saveProjectOverview(true)`) is the only save path; the `#ovStatus` "Saved" flash span stays — [Overview]
...
- `quickAddSectionHTML()` / `quickAddOpen` / `toggleQuickAddSection()` (Phase Quick Add) — the collapsible "Quick add" box on Overview, between Project details and Tasks. Same `sd-block-head`/`dept-caret` idiom and collapsed-by-default convention as Tasks/AI Scan. Three buttons (Shoot date / Location / Crew), one control open at a time (`quickAddMode`), each opening the minimum fields for one record. **Add-only and deliberately shallow** — it's a shortcut into data that has full editing on its own tab, not a fourth place to manage any of it. No bulk add, no editing, no AI. It shares nothing with AI Scan except the accept-side functions below — [Overview]
- `setQuickAddMode()` / `resetQuickAdd()` — mode switching and the return-to-rest reset. Clicking the already-open control closes it, so the box never needs a Cancel button. `resetQuickAdd()` is also called from `addLocToProject()`, `addCrewToProject()` and `saveLocation()`'s project branch, so the box settles back to three buttons whichever shared function the user actually finished through — [Overview, Crew, Locations]
- **Quick add reuses, it does not reimplement** (the point of the phase — a divergent second implementation of any of these three would be a bug):
  - shoot day → `addShootDayFromProposal()` (via `quickAddShootDay()`) — the same function AI Scan's `propose_shoot_date` accept path calls, which itself reuses `makeShootDayRecord()`
...
| T-1.8 | · Quick add | Collapsible add-only shortcut for one shoot date / location / crew member, reusing AI Scan's and Add Location's own commit functions (Phase Quick Add). Sits here on screen but numbered .8 — codes are stable handles, allocated when a section is built, so existing references keep pointing at the same thing | `quickAddSectionHTML()` |
