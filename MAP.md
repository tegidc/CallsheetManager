...
- `crewRolesRowHTML()` — renders one person's row in the Roles grid: CHECKBOX | EDIT | Name | Role (saved-roles taglist + add-role picker) | Show as. Department and Lead Company are NOT edited here any more (Phase R) — department is implied by the active saved role (see `setActiveRole`), Lead Company only lives in the per-person edit expansion (`crewFormHTML`) or the bulk-edit panel — [Crew]
- `showAsQuickEditHTML()` / `saveQuickShowAs()` — inline quick-edit for a crew member's "Show as" cosmetic override, used on the Roles row — [Crew]
- `roleQuickEditHTML()` / `saveQuickRole()` — render and save the inline quick-edit for a crew member's active role (Days on site grid); always a select of their saved roles now, never free text — [Crew]
