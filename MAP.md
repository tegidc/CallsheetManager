...
- `shootDayEditCardHTML(d)` (replaces `shootDayBuildHTML()`, 19 Sep 2026) — the call sheet card as an editor: title line, a date / Day # / all-crew-call sub-line, the two-column `.key-info-grid` holding weather + location + hospital + car park (left) and brief + parking + notes (right), then Additional locations, Schedule (all row tools; the hover **+** sits at `left:70px`, clear of the move arrows) and Position assignments, with the feedback button and a quiet "Delete this shoot day" link at the foot. **General information** (the grid + Additional locations), **Schedule** and **Position assignments** each collapse from their own `h3.sd-sec-toggle` — keys `general`/`schedule`/`positions` in `previewBlocks`, on the shared `togglePreviewBlock()` plumbing, so Collapse all covers them and the state carries across days. ⚠️ Collapsed = `display:none`, never removed: `saveShootDay()` still reads those fields. `shootDayDblEdit()` re-opens all three so the target field can take focus. ⚠️ Every field id is the old form's (`sdDate`, `wxTemp`, `sdLocation`, `rows-sched`, `rows-posn`, …) — `populateShootDay()`/`saveShootDay()` are unchanged. `#sdBrief` still auto-grows (`autoGrowTextarea`) — [Shoot Days]
- `shootDayHeaderRowHTML()` — the Crew-tab-style header row: **Edit/Editing** toggle, **Include** toggles (one `.bnr-tool` per `exportSections` key — what shows below the card AND what Copy/Export carry), then Collapse all (`#eca-pv`, `toggleAllPreviewBlocks()`), **Copy WhatsApp** (`copyExportOutput()`) and **Export .xlsx** (`downloadExcel()`), plus the `#sdStatus` save flash. Replaced the fold-out Format panel; there is no WhatsApp VIEW any more, only the copy — [Shoot Days]
...
| **T-5** | **Shoot Days** | The per-day detail AND its outputs — one page since 19 Sep 2026 (absorbed T-7). Opens in Edit | `renderProjectDays()` |
