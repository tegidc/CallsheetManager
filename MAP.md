...
- `crewSelectMode` / `toggleCrewSelectMode()` / `crewToolbarSelectAll()` — **Crew tab SELECT MODE (19 Sep 2026), replacing the per-row tick box.** The toolbar's Select toggle turns the list into a picker: a capture-phase click listener in `renderProjectCrew()` takes any click on a `.pb-head` line, flips that row's (hidden) `.bulk-select-cb` and fires its change handler — so `projectCrewSelected`, `toggleCrewPersonSelected()`, the bulk action bar and `selectedEditTargets()` are all unchanged. ⚠️ The checkboxes are still rendered, `display:none` — they are the plumbing, not dead markup. A selected line is a highlighted line (`.pb-head:has(.bulk-select-cb:checked)`, an opaque `color-mix` so the sticky cells take it too). In select mode `.pb-head *{pointer-events:none}` so a click is only ever a selection. Leaving select mode KEEPS the selection (rows editable again; an edit to one still applies to all). Desktop only, the freed column closes up: `--pb-controls-w` 0 on Roles, 24px on the views that still keep an Edit pencil there; ≤900px the column is a hard 60px, so the variable is left alone — [Crew]
