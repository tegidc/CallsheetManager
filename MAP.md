...
- `buildTaskFlagGroups()` / `buildTaskFlags()` / `taskFlagPillsHTML()` / `syncFlagTasks()` — **flags, reworked 20 Sep 2026.** A flag GROUP is one pill ("3 crew members with no day rate set"); its ELEMENTS are the individual things behind it, each with a stable key: `date:<dayId>`, `loc:<dayId>`, `daycrew:<dayId>`, `rate:<entryId>`, `venues`, `crew`. `buildTaskFlagGroups()` computes everything; `buildTaskFlags()` is what is SHOWN — minus `p.ignoredFlags` and minus any element that already has an open task (`t.flagKey`) — with the pill re-counted via the group's `titleFn`. A pill opens the flag popup (no longer navigates). `syncFlagTasks()` runs at the top of `renderTasksSection()` and ticks off any open flag-task whose problem is no longer live — fixed from its own field, fixed anywhere else in the app, or its subject deleted — [Overview]
- `openFlagDialog()` / `closeFlagDialog()` / `renderFlagDialog()` / `flagApply()` / `flagUndo()` / `flagAdd()` / `flagIgnore()` / `flagAddAll()` / `flagIgnoreAll()` / `liveIgnoredFlagCount()` / `restoreIgnoredFlags()` — **the flag popup**: one row per element with **Add** (pushes a task `{…, flagKey}` and opens the Tasks section) or **Ignore** (`p.ignoredFlags`), Add all / Ignore all, "Open the page instead" (the old click-to-fix navigation), Esc/Close. ⚠️ **Stays open until Done** (Esc / click outside also close) so several elements can be worked through in one sitting: the rows are a snapshot taken on open (`flagDialogRows`), and a handled row stays in the list marked "✓ Added to tasks" / "Ignored" with **Undo** (`flagUndo()`, `flagDialogHandled`); scroll position is kept across re-renders. Body-level element `#flagDialog`, like `#exportDialog`. "N ignored · Restore" under the pills clears `p.ignoredFlags` — [Overview]
- `taskFieldHTML(t)` / `completeFlagTask(taskId)` — **a flag-task carries the field that fixes it**, so nothing needs a page flip: `rate` → £ input → `e.rate` (the same field Budget → Per Person writes); `date` → date input → `d.date`; `loc` → venue select (this project's first) → `d.locationId` (+ added to `p.locationIds`, parking pre-filled as the day form does); `daycrew` → crew select → `d.positions`; `venues` → venue select → `p.locationIds`; `crew` → "+ Add person" (opens the Crew tab's popup; ticks itself off via `syncFlagTasks()`). Done / Enter validates (message beside the field), writes, ticks the task, renders, then saves. Deleting an open flag-task turns it back into a flag — [Overview]
...
- `renderProjectBody()` / `renderProjectBodyNow()` / `SCROLL_KEEPERS` — **every re-render keeps your place (20 Sep 2026).** A tab is rebuilt wholesale by innerHTML, which used to shift the page and snap any inner scroll box (Roles' frozen-pane `.roles-tablewrap` above all) back to its top. `renderProjectBody()` now records `window.scrollX/Y` and each `SCROLL_KEEPERS` element's scrollTop/Left, calls `renderProjectBodyNow()` (the old dispatch-by-tab body), and restores them — only when it is still the same screen (`currentProjectId|tab|crewGridView|budgetView`); a tab change still starts at the top (`goProjectTab()` scrolls to 0 after). — [Shared]
...
- `notesSectionHTML()` / `wireNotesHandlers()` / `toggleNotesSection()` / `flushNotesEdits()` — **Notes is a collapsible SECTION OF OVERVIEW since 20 Sep 2026** (`#notesSection`, open by default, after Tasks); the NOTES tab and `renderProjectNotes()` are gone, and a stale `currentProjectTab==='notes'` is rerouted to Overview. The Overview's `body.oninput` routes `.note-text` typing to `noteAutoList()`/`noteAfterEdit()`, leaves the Tasks/Notes sections' own fields alone, and autosaves project details for everything else. ⚠️ `saveProjectNotes()` READS THE EDITORS — `flushNotesEdits()` is called before anything rebuilds or leaves them (`renderProjectOverview`, `goProjectTab`, `toggleNotesSection`, `addProjectNote`), or the last second of typing is lost — [Overview]
...
- `sanitizeNoteHTML()` / `noteHTML()` / `NOTE_TAGS` — rich-text notes (18 Sep 2026). The ONE gate for note markup: rebuilds from a tag whitelist (b/i/u, ul/ol/li, div, br, h3), no attributes ever. Runs on paste, on save and on render — never put `n.html` into the page except through `noteHTML(n)` — [Notes]
- `noteCmd()` / `noteAfterEdit()` / `refreshNoteTools()` / `noteAutoList()` — the per-note toolbar (B, I, H, bullets, numbers via `execCommand`), active-state highlighting, and the "- " / "* " / "1. " start-of-line shortcuts. Tab / Shift+Tab nesting, Enter-on-empty-item to leave a list, and sanitised paste live on `body.onkeydown` / `body.onpaste` inside `renderProjectNotes()` — [Notes]
- `addProjectNote()` / `saveProjectNotes()` / `deleteProjectNote()` — the three writes. A note is `{id, html, text, createdAt, updatedAt}` on `p.notes` (`html` = rich body, `text` = its plain-text shadow for search / the Undo peek / pre-18-Sep notes) (`db:projects`), next to `p.tasks`. Add renders first and saves in the background (G14); save reads every body on screen and writes only the ones that differ, stamping `updatedAt`; delete goes through `beginUndo`/`finishUndo` — [Notes]
...
- `renderTechSpecsSection()` — renders T-7.3 into `#techSpecsSection` from `renderProjectDays()`, as a `.card`. **Follows `shootDayEditing` since 19 Sep 2026:** finished view = only `#techSpecsExported` (double-click → Edit); Edit = hint, `#techSpecsBanner`, the six fields (`techSpecFieldsHTML('ts')`), "Use for every day", the camera editor (`#cameraDesigList`) and `#techSpecsExported`. Fills the fields from `resolveTechSpecs(day)` after rendering — [Tech Specs, Preview & Export]
- `saveDayTechSpecs()` — the autosave target for the six fields (wired in `renderProjectDays()`'s `body.oninput` by field id, alongside the Travel/Hotel cost autosaves). Stores `overlayDiff(entered, projectDefaults, TECHSPEC_FIELDS)` on the day, deleting the key when nothing differs — the same rule `saveShootDay()` used to apply — [Tech Specs]
...
- `renderProjectDays()` — **the merged Shoot Days page (19 Sep 2026 — the Preview & Export tab T-7 was folded into it).** Day tabs + `shootDayHeaderRowHTML()` + ONE call sheet card in one of two states + the included spec blocks (tech / hotels / catering / travel, each a `.card`). `shootDayEditing` (default **true**) picks the card: `shootDayEditCardHTML()` (the editor — `.card.editing#sdEditCard`, filled by `populateShootDay()`) or `#previewCard` (`.card.viewing`, drawn by `renderPreviewCard()`). Its `body.oninput` routes by field id — Travel/Hotel cost fields, the six tech-spec fields (`saveDayTechSpecs()`), and anything inside `#sdEditCard` → `saveShootDay(false)`. A **capture-phase** click listener autosaves on any button press inside the card (row ×/move/shift/insert fire no input event, and there is no Save button any more; capture because a row's × detaches its own target before the click bubbles) — [Shoot Days]
...
- `shootDayEditing` / `setShootDayEditing()` / `flushShootDayEdits()` — the page's Edit state. ⚠️ `saveShootDay()` READS THE FORM, so anything that re-renders or leaves the edit card calls `flushShootDayEdits()` first (`selectShootDay`, `setShootDayEditing`, `toggleExportSection`, `goProjectTab`, `copyExportOutput`, `downloadExcel`; `addShootDay` AWAITS it so its own `db:shootdays` write doesn't race the flush). `saveShootDay()` itself returns early when `#sdEditCard` is absent — without that guard a late autosave on the finished view would blank the day. REMOVED with the merge: `shootDayBlocks`, `toggleShootDayBlock()`, `setAllShootDayBlocksCollapsed()`, `sdBlock()`, the sticky Save bar — [Shoot Days]
- `shootDayEditCardHTML(d)` — the **Call Sheets** tab's edit card (the tab was "Shoot Days" until 19 Sep 2026; route key is still `'days'`). ⚠️ **It is the finished card with fields in it:** same title/sub line, the same three sections (`sdSectionOpenHTML()` — General information / Schedule / Position assignments, keys `general`/`schedule`/`positions` in `previewBlocks`, so what is open stays open across Edit/Done and Collapse all covers them), the same `.key-info-grid`, the same fixed-width tables (`SD_SCHED_COLS` / `SD_POSN_COLS`, shared with `renderPreviewCard()`). Plain text fields are type-matched to the text they replace (`#sdEditCard input…` CSS: no padding, inherited font, underline as a box-shadow); inline fields size to their content (`sdAutoSize()`, measured on a canvas; `sdAutoSizeAll()` after populate/fetch); textareas auto-grow. Tools are small: `.ki-tool` icon buttons with a `data-tip` hover label replace the Fetch buttons (weather, hospital, car park); schedule rows get a hover tools cell (`.sd-rowtools`: tick / move / ×) and the insert **+** in the card's left gutter on the row boundary. ⚠️ Every field id and row class is the old form's (`sdDate`, `wxTemp`, `sdLocation`, `#rows-sched tr.row` with `.sc-time/.sc-dur/.sc-desc/.sc-select`, `#rows-posn tr.crewrow-select` with `.posn-crew/.posn-call`) — `populateShootDay()`/`saveShootDay()`/`rowsOf()` are unchanged. Collapsed = `display:none`, never removed, so a save still reads those fields. ≤900px the edit tables scroll sideways in `.sd-tablescroll` — [Call Sheets]
- `positionGroups(d)` / `renderPositionAssignments()` / `positionsTableHTML()` — `positionGroups()` is the ONE grouping + order of people on a sheet (Client, Crew, Talent, co-production companies; a lone Crew group goes unlabelled), read by both states. `renderPositionAssignments()` (edit) draws those tables with the call time as a field, the NAME as the way in to the per-day override form (`editCrew()` → `dayOverrideFormHTML()` in a row beneath, as on Crew → Roles — no pencil) and a hover ×. REMOVED 19 Sep 2026: `posnIdentityHTML()`, `collapsedPosnCompanies` / `collapsedPosnDepts` / `togglePosnCompany()` / `togglePosnDept()` — [Call Sheets]
- `shootDayHeaderRowHTML()` — the Crew-tab-style header row: **Edit/Editing** toggle, **Include** toggles (one `.bnr-tool` per `exportSections` key — which blocks show below the card), then Collapse all (`#eca-pv`, `toggleAllPreviewBlocks()`) and ONE **Copy / Export** button (`openExportDialog()`), plus the `#sdStatus` save flash — [Call Sheets]
- `openExportDialog()` / `closeExportDialog()` / `renderExportDialog()` / `exportDialogCopy()` / `exportDialogExcel()` / `activeExportSections()` — **the Copy / Export popup.** Holds its OWN copy of the four section switches (`exportDialogSections`), seeded from the page's Include toggles on open, so choosing what goes out never rearranges the page. `buildWAText()` and `downloadExcel()` read `activeExportSections()` — the popup's copy while it is open, `exportSections` otherwise. Copy for WhatsApp / Export .xlsx / Close; Esc closes. `copyExportOutput()` is gone — [Call Sheets]
- `shootDayDblEdit(ev)` — double-click on the finished card → Edit, focused on the clicked field. `renderPreviewCard()`/`positionsTableHTML()` tag what is clickable with `data-edit`: `'#id'`, `'sched:row:col'`, `'posn:entryId'` (hence `id` on `buildFullData()`'s position rows) or `'tech'` — [Shoot Days]
...
- **Fifth pass:** no "Final total" word under the big figure (it is the `title`); the three lines under it are
...
- ⚠️ **TALENT ETC (T-10, 20 Sep 2026) — Talent and Client are not crew.** `TALENT_ETC_DEPTS` / `isTalentEtc(v)` (anything with a `department`: crew record, entry, entry view) is the ONE test. They are still crew-database records and still project entries — so shoot-day positions, call times and the call sheet's own Client / Talent blocks are untouched — but inside a project: they live on their own tab (**TALENT ETC**, `renderProjectTalent()`, route `'talent'`); `renderProjectCrew()` filters them out of every Crew view EXCEPT Catering (a lunch is a lunch); `resolveEntryRate()` gives them a rate ONLY when one is typed on this project (never the database fallbacks); the "no day rate" flag skips them; `buildBudgetData()` drops any with no fee (no £0 rows) and adds `talentEtcCosts(p)` to the Hotels / Travel lines (project-wide, on no single day). The Crew tab's "Exclude Talent" filter is gone; Budget's is now "Exclude Talent etc" (`personMatchesProjectFilter` uses `isTalentEtc`). — [Talent etc]
- ⚠️ **THE PAGE HEAD (20 Sep 2026) — Crew and Talent etc share one layout.** `pageHeadHTML({budgetRowHTML, totalsHTML, toolsHTML, stageHTML, chipHTML, noteHTML})` (CSS `.phase-banner.bnr-head`, `.bnr-left`): LEFT — Budget (+ VAT / Split kit / Costs PP) on top, the total in a RESERVED one-line slot, and `toolGroupHTML()`'s joined **+ Add | Filter | Select** on the bottom line; `.bnr-left` has a min-height and `space-between`, so the head is the same height with Budget on or off. RIGHT — the stage selector, on EVERY sub-tab of both tabs (nothing moves when you switch), and it FILTERS every sub-tab: `entryInSelectedStage(entryId, p)` is the one row test (Hotel / Travel / Catering keep their on-shoot rule on top of it). `▾ Stages` folds the whole head to one line (`setHeadFolded()`, `appSettings.headFolded` in `db:settings`); the `.stage-chip` that stands in for the selector is coloured by the selection via `--tape` / `applyStageColour()` — black for all stages or a mix (a mix also shows its dots), yellow / green / purple for one isolated stage — and clicking it unfolds. `budgetSummaryBarHTML()` returns this shape when its caller passes `o.listToolsHTML`; `o.peopleKind` ('crew' | 'talent') scopes its figures through `stageFiguresFor(data, kind)` — money from the budget build's people, head-counts from the entries (a no-fee client still counts), and Talent's Production figure includes the hotels and travel we book (`talentEtcCosts`). The Budget tab passes neither and keeps the `TOTALS | STAGE | TOOLS` banner. Sub-tabs come AFTER the head: `crewSubTabsHTML(current, setterName, rightHTML)` — **Roles / Travel / Catering / Hotel** — with Summary / Collapse all at its right-hand end; on Roles the row sits inside `.roles-zone`. Removed: the "Crew on this project" heading, the "Rates and totals hidden…" note, the Roles tips (`rolesFootnoteHTML()` renders only the custom-column-order "put them back" line; `setRolesTipsHidden()` is gone, `appSettings.hideRolesTips` is no longer read). Mock-up: `demo-page-head.html`.
- **Filter is a button and a popup on Crew and Talent etc (20 Sep 2026):** `filterButtonHTML({count, onToggle, inverted})` + `filterDialogHTML(panelHTML, onClose)` wrap the ONE `filterPanelHTML()` panel in a `.modal-box.filter-box` (in `SCROLL_KEEPERS`). Both tabs share `projectCrewFilter`. Budget and the Crew Database still use the `filterToggleHTML()` disclosure. Select mode works on both tabs — `hookCrewSelectClicks(body)`; `goProjectTab()` clears the selection, select mode and the filter popup when the tab changes.
- ⚠️ **CATERING IS ONE LINE PER PERSON AND ONE KITCHEN (20 Sep 2026).** `crewCateringBlockHTML()` puts three small ticks (B L D, `.meal-trio`; header from `cateringDayHeadsHTML()`, column width `CATERING_DAYCOL`) inside each day column on the person's own line — the three Breakfast / Lunch / Dinner sub-rows are gone; the row's tail is the person's **Dietary** text (`.meal-diet` — anything other than the standard `DIETARY_OPTIONS[0]` diet plus `dietarySpecific`, under a `CATERING_DIET_HEAD` heading), Shared | Own for talent, then **All** — a disclosure (`mealAllOpenFor` / `toggleMealAllOpen()`, one row open at a time) that reveals B / L / D, each calling `toggleAllMealForPerson`. The header's B / L / D letters are controls too: `toggleMealColumn(dayId, mealKey)` ticks (or, if all are on, unticks) that meal on that day for everyone LISTED beneath it — read from the rows on screen via the inputs' `data-crew` / `data-day-id` / `data-meal`, so the filter, the stage selector and a folded group (hidden rows are left alone) are all respected. BOTH tabs use it: each lists its own people and folds the other side underneath (`cateringOtherSideHTML(showTalent, days)`, `cateringOtherCollapsed`), over the one project-wide `cateringSummaryHTML()`. Talent / client rows carry **Shared | Own** — `e.ownCatering` on the project entry, read person-level by `crewOwnCatering(crewId, p)`, written by `setCrewOwnCatering()`: Own keeps the ticks but drops the person from `buildCateringSummaryGrid()` (covers, dietaries, cost) and from their `mealCost` in `buildBudgetData()`.
- **AUTO-FILL MEALS (20 Sep 2026)** — the `Auto-fill meals` button on the Catering sub-tab row of both tabs (`mealFillButtonHTML()` → `mealFillDialogHTML()` → `applyMealFill('gaps' | 'all')`). Rules in `MEAL_RULES` / `suggestedMeals(callMins, wrapMins)`: call before 08:30 → breakfast · on site across 13:00 → lunch · wrap after 18:00 → dinner. Call = `personCallMins()` — the person's own call on the day's Position assignments (`pos[1]`, earliest of their entries), else `d.allCrewCall`; wrap = `dayWrapMins()` — the schedule's WRAP row; times read by `looseTimeToMins()` (`08:00`, `0800`, `8am`, `6.30pm`). ⚠️ THE BACKUP: no call and no wrap → LUNCH ONLY; only one of them → decides what it can and assumes lunch. Covers crew AND talent whichever tab it is opened from (`mealFillPeople()` — everyone on the project except Own catering), only on days they are booked (`crewOnDay`). `gaps` touches only person-days with nothing ticked; `all` re-works everything and clears days they are not booked on. Undoable (`beginUndo(['db:shootdays'])`).
- ⚠️ **ALL | CREW | TALENT ETC (24 Sep 2026) — the TALENT ETC tab is gone; it is a switch on Crew.** `crewPeopleKind` ('all' | 'crew' | 'talent', default 'all'; the last one picked is remembered in `appSettings.crewPeopleKind`, db:settings) + `setCrewPeopleKind()` (clears the selection) + `peopleKindSwitchHTML()` — a joined `.bnr-group` at the head of the page head's tools row, before + Add | Filter | Select. `renderProjectCrew()` hands 'talent' straight to `renderProjectTalent()` (the old tab, unchanged). CREW is the crew list as before. ALL is the crew list with `talentSectionHTML()` beneath it — its own folding section (`talentSectionCollapsed` / `toggleTalentSection()`), its own columns, because a talent row (fee, own hotel/travel) is not a crew row; its rows join Select all. Catering on ALL is ONE list (everyone in `assigned`, no `cateringOtherSideHTML`). `talentListParts(p, days)` is the one talent-list build both callers use. `talentView` / `setTalentView()` are gone — `crewGridView` drives both lists. The head on ALL: `peopleKind:'all'` → `stageFiguresFor(data,'all')` (everyone + `talentEtcCosts`), labelled "Crew & talent total"; `headDeptFigs(null)`. + Add person on ALL is `openAddPerson('all')` — searches everyone and offers both creates (crew form / three-field talent form). The route `'talent'` still works: `goProjectTab('talent')` and `renderProjectBodyNow()` land on Crew with the Talent etc switch. The body's `oninput` summary savers skip `.talent-section` inputs (they save themselves). — [Crew / Talent etc]
- `renderProjectTalent()` / `talentColumns(days)` / `talentRowHTML(v, days, cols)` / `talentGroupHTML(kind, views, days, cols)` / `talentView` + `setTalentView()` / `collapsedTalentGroups` + `toggleTalentGroup()` — **Talent etc is split across the Crew sub-tabs (20 Sep 2026), under the same page head (Budget + stages throughout):** Roles (Prep / day ticks / Post — the SAME numbers Crew's Roles writes via `setPhaseDays`, so talent have stages — plus Fee), Travel (`e.trip`), Catering (see above), Hotel (`e.stay`). Money columns (fee, per night, cost, totals) and the `#talentSummary` line follow the Budget toggle (`crewShowMoney`), as on Crew. Rows are `.tegrid` — `.crewgrid`'s lead columns, header type and tick size, middle columns per view as `--te-cols`; `body.talent-tab` sets one pair of name / role widths so `.tegrid` and Catering's `.crewgrid` line up. `talentView` is part of `renderProjectBody()`'s screen key. Unchanged savers: `setTalentFee()` / `setTalentField()` / `removeTalentFromProject()` / `stayOf()` / `tripOf()` / `talentEtcCosts()`.
- `renderProjectTalent()` / `talentGroupHTML()` / `talentSummaryHTML()` / `refreshTalentSummary()` / `setTalentFee()` / `setTalentField()` / `removeTalentFromProject()` / `stayOf()` / `tripOf()` / `talentEtcCosts()` — the tab: one call-sheet-style card, a Client group and a Talent group. Per person: name (opens their record — `editCrew()` → `crewEditModalHTML()`, rendered on this tab too; `refreshCrewScreen()` re-renders it), role, a tick per shoot day (`toggleCrewOnDay()` — the SAME `d.positions` the call sheet reads), an optional **Fee** (`e.rate` + `e.buyout:true`, so Budget costs it once; cleared = both deleted), and beneath it their OWN hotel and travel — `e.stay = {hotel, nights, perNight, by}`, `e.trip = {note, cost, by}`, `by:'us'|'them'` (default us for Talent, them for Client). Only `by:'us'` is costed. Fields autosave (`scheduleAutosave('talent')`), `#talentStatus` flashes. — [Talent etc] **(Superseded 20 Sep 2026 — see the entry above; the card and `te-table` are gone.)**
- `addPersonKind` / `addPersonEveryone` / `setAddPersonEveryone()` / `addPersonCreateJS()` / `startNewTalentFromAddPerson()` / `addTalentNewFormHTML()` / `saveNewTalent()` — **the add-person search is SCOPED**: `openAddPerson()` (Crew) finds crew only, `openAddPerson('talent')` finds talent and clients only, via `crewSearchResultsHTML()`; when a query has matches outside the scope a "N more … — search everyone" row lifts it. Talent etc's "create new" is a three-field form in the popup (name, Talent/Client, role → `addNewCrewFromProposal()`), not the full crew form. Overview's Quick add is a crew search. — [Talent etc / Crew]
- `isolatedStage()` / `allStagesOn()` / `onlyBudgetScope(key)` / `allBudgetScopes()` / `applyStageColour(body)` / `STAGE_CLASS` / `stageTagsHTML()` / `rolesTagsAsText()` — **the stage selector can isolate as well as mute (20 Sep 2026).** The banner's stack (shared by Crew → Roles and Budget, built in `budgetSummaryBarHTML()`) is now: an **All stages** row (one click = everything back; shows every stage's figures / total days), then Pre-production / Production / Post, each `dot | name | ONLY | figures` — the name column is a fixed width so the three **Only** buttons stand in one vertical line straight after the titles. Row click still mutes/unmutes (`toggleBudgetScope()`); **Only** shows that stage alone, pressed again = everything. `isolatedStage()` is the ONE definition of "a single stage is showing". ⚠️ **The stage colour:** `applyStageColour()` puts `stage-all|pre|prod|post` on `#projectBody` (Crew → Roles and Budget), and those classes RE-POINT the brand tokens — `--tape` and the six `--tint-*` — at ink black (All stages or a mix) or the stage's own colour (`--phase-prep` / `--phase-shoot` / `--phase-post`). Everything that was brand-green on those tabs follows without knowing about stages; the dots and Only buttons always wear their OWN stage's colour. `stageTagsHTML()` puts small stage tags beside each name on Roles — only for stages currently showing — as words where the list has room (`rolesTagsAsText()`: money off, ≥901px; the name column is widened inline to take them) and dots otherwise — [Crew / Budget]
- ⚠️ **Crew's Pre-Prod, Production and Post tabs were RETIRED 20 Sep 2026** — `viewLabels` in `renderProjectCrew()` is Roles / Hotel / Travel / Catering, and a stale `crewGridView` of `'preprod'|'days'|'post'` is coerced to `'roles'` at the top of that function. Roles covers them: the stage selector filters, Prep/Post columns hold the numbers, **Shoot** opens the day ticks (`toggleRolesShootColumn()`), **Dates** opens the calendar. The views' render code (`isPhaseTab()` branch, the `'days'` crewgrid, `PHASE_TABS`) is still in the file but UNREACHABLE — candidate for removal; check what Hotel/Travel/Catering and the phase calendar share with it first. Budget never had stage tabs. — [Crew]
...
- `exportSections` / `EXPORT_SECTION_LABELS` / `toggleExportSection()` (Phase Q) — the four Include switches (tech / catering / hotels / travel) gating the blocks on the Shoot Days page, the WhatsApp text and the .xlsx sheets alike. Driven by the header-row toggles since 19 Sep 2026. REMOVED then: `exportFormat` / `setExportFormat()` (no WhatsApp view — `exportOutputText()` always was `buildWAText()`), `exportPanelOpen` / `toggleExportPanel()` / `exportHiddenSectionCount()` / `exportOptionsHTML()` / `exportPanelBodyHTML()` — [Shoot Days]
...
- ~~`renderProjectPreview()`~~ — REMOVED 19 Sep 2026; the tab is gone and `renderProjectDays()` renders everything it did. A stale `currentProjectTab==='preview'` is rerouted to `'days'` in `renderProjectBody()` — [Shoot Days]
...
- `loadDB()` / `saveDB()` — the single choke point for reading/writing any app data collection to/from Supabase, keyed by a `db:*` string. **Since Phase AR (G11) `saveDB()` returns a result — `{ok:true}` or `{ok:false, kind, detail}` — and raises the failure banner itself.** The ~80 call sites that ignore the return are still correct; don't add per-call-site error handling (same rule R18 set) — [Shared/utility functions] **20 Sep 2026: `saveDB()` is a per-key QUEUE in front of `saveDBNow()` (the guarded write).** Two saves of one key in flight together used to race each other — the second "beaten" by the first — and a burst of quick clicks could lose all three retries and raise the "not saved" banner against itself. Same-key saves now run one after another (`_saveChains`); `value` is the live array, so a queued save writes the state as it is when its turn comes. The merge path remains for a genuine second tab/device.
...
- `crewIdentityHTML()` / `crewControlsHTML()` / `deptLabelHTML()` / `posnIdentityHTML()` / `crewExpansionHTML()` — shared rendering helpers for how a crew member's identity/role/department are displayed across tabs. `crewIdentityHTML()`'s department badge is always the read-only `deptLabelHTML()` now (no more editable department pill — see Phase R item 1). `opts.hideDept` / `opts.hideLeadPill` suppress the department badge / Lead Company pill (used by Days on site/Hotel/Travel/Catering — Phase S item 6; the Roles tab doesn't use this function at all any more, see `crewRolesRowHTML`). ⚠️ **`opts.selectCbHTML` (Phase BK) REPLACED `opts.bulkSelect`** — do not look for the old one: every Crew list is one person BLOCK per person now, and the ids a block's checkbox must toggle are that person's currently VISIBLE entries, which only the block knows (a partial-match filter can hide some). The old branch built the checkbox from `c` alone and had no way to express that. ⚠️ `opts.showAsOrRole` and `opts.wrap` were REMOVED (15 Aug 2026 cleanup) — Phase BO moved the role chip into its own grid column via `roleBannerHTML()`, after which no caller passed either option. (`posnIdentityHTML()` was removed 19 Sep 2026 — see `renderPositionAssignments()`.)
- `crewSelectMode` / `toggleCrewSelectMode()` / `crewToolbarSelectAll()` — **Crew tab SELECT MODE (19 Sep 2026), replacing the per-row tick box.** The toolbar's Select toggle turns the list into a picker: a capture-phase click listener in `renderProjectCrew()` takes any click on a `.pb-head` line, flips that row's (hidden) `.bulk-select-cb` and fires its change handler — so `projectCrewSelected`, `toggleCrewPersonSelected()`, the bulk action bar and `selectedEditTargets()` are all unchanged. ⚠️ The checkboxes are still rendered, `display:none` — they are the plumbing, not dead markup. A selected line is a highlighted line (`.pb-head:has(.bulk-select-cb:checked)`, an opaque `color-mix` so the sticky cells take it too). In select mode `.pb-head *{pointer-events:none}` so a click is only ever a selection. While anything is selected the toolbar shows a **Clear selection (N)** button beside Select (in or out of select mode), and Select all is its own button in select mode. Leaving select mode KEEPS the selection (rows editable again; an edit to one still applies to all). Desktop only, the freed column closes up: `--pb-controls-w` 0 on Roles, 24px on the views that still keep an Edit pencil there; ≤900px the column is a hard 60px, so the variable is left alone — [Crew]
...
| T-1.4 | · Tasks — Flags | **20 Sep 2026: a pill opens a popup — Add each element to the task list or Ignore it.** Live, computed warning pills — the original three (missing dates/locations/crew) plus three toggle-able rules (Phase Tasks) | `buildTaskFlags()` |
| T-1.5 | · Tasks — Manual + flag tasks | Title/note/tick, persisted per project; a task added from a flag carries the field that fixes it (`taskFieldHTML()`) | `addProjectTask()` |
...
| **T-5** | **Call Sheets** (was Shoot Days) | The per-day detail AND its outputs — one page since 19 Sep 2026 (absorbed T-7). Opens in Edit | `renderProjectDays()` |
| T-5.1 | · Day tabs | Day switcher + New day | `dayTabsBarHTML()` |
| T-5.1b | · Header row | Edit toggle, Include toggles, Collapse all, Copy WhatsApp, Export .xlsx | `shootDayHeaderRowHTML()` |
| T-5.2 | · Call sheet card | Edit state: the card with fields where they print (`shootDayEditCardHTML()`); finished state: `renderPreviewCard()` — Client, Position assignments, Talent, co-production groups; double-click to edit | `shootDayEditCardHTML()` / `renderPreviewCard()` |
...
| **T-9** | **Notes** (a section of Overview since 20 Sep 2026 — no longer a tab) | Per-project scratchpad: rich-text notes (paragraphs, bold/italic, heading, bullet + numbered lists — 18 Sep 2026), newest first, searchable, autosaved (14 Sep 2026). No tags or attachments by design | `renderProjectNotes()` |
| **T-10** | **Talent etc** (a switch on Crew since 24 Sep 2026 — All \| Crew \| Talent etc; no longer a tab) | Talent and Client on a project, apart from Crew: day ticks, optional fee, their own hotel and travel per person, scoped add-person search. On All they are a section under the crew list | `renderProjectTalent()` / `talentSectionHTML()` |
| T-9.1 | · Note row | Created stamp (+ "edited" stamp when it differs), delete with Undo, formatting tools (shown on hover/focus), borderless contenteditable body | `renderNotesList()` / `saveProjectNotes()` |
| ~~T-7~~ | ~~Preview & Export~~ | Folded into T-5 on 19 Sep 2026. The T-7.x rows below are now blocks on the Shoot Days page, each wearing the call sheet `.card` | `renderProjectDays()` |
| T-7.0 | · ~~Format panel~~ | Replaced by the header row (T-5.1b) | — |
...
| T-7.2 | · WhatsApp text | Copy only (header row) — no on-page view. Same Client/Positions/Talent/co-production ordering as the card | `buildWAText()` |
